...
#%% Load Data
bas_train_feats = np.load('BAS_train_feats.npy')
bas_train_labels = np.asarray('BAS_train_labels.npy') #I don't need the labels here 


# #############################################################################
#%% Training

#can go up to 66 hidden unites according to Koshka paper. Hopefully can do less though
n_components = 50
learning_rate = 0.01
batch_size = 50
n_iter = 150
...
np.save('trained_rbm/bas_rbm_components_.npy', rbm.components_)
np.save('trained_rbm/bas_rbm_intercept_hidden_.npy', rbm.intercept_hidden_)
np.save('trained_rbm/bas_rbm_intercept_visible_.npy', rbm.intercept_visible_)
