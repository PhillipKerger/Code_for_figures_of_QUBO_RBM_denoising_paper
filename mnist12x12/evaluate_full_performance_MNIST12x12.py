...
"""
This script tests the denoising method across the test set and different
noise levels. 
...
#noise range to use
noiseinc = 0.01 #noise increments (decrease for a finer plot)
noisemin = noiseinc   #noise min value to start at
noisemax = 0.32  #max noise to stop at
...
testnum = 200 #number of images tested at each noise level -- increase this for better confidence regions 
...
test_feats = np.load('mnist12x12_testfeats.npy')
test_labels = np.load('mnist12x12_testlabels.npy')
...
folder = 'mnist_trained_rbm/'
rbm.components_  = np.load(folder +'mnist12_rbm_components_.npy')
rbm.intercept_hidden_ = np.load(folder + 'mnist12_rbm_intercept_hidden_.npy')
rbm.intercept_visible_ = np.load(folder +'mnist12_rbm_intercept_visible_.npy')  
...
savedatafolder = 'data_from_full_eval_mnist12/' #to save all data from the next loop
...
for i,sigma in enumerate(sigma_range):
     ...
     #qubo denoise with approximate sigma
...
ax.legend(['QUBO (exact $\sigma$)','_nolegend_',
           ...
           'Gaussian Filter','_nolegend_','Median Filter','_nolegend_',
            'Minmax Flow','_nolegend_', 'Gibbs Denoising'])
plt.ylabel('Proportion of Correct Pixels')
plt.xlabel('noise level $\sigma$')
plt.title('Comparison of Methods on MNIST12')
